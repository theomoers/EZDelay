...
class BPWEmissionBaseline(EmissionBaseline):
    ...
    def baseline_emission_setup(self):
        ...
        *Raises ValueError if baseline_num is outside acceptable range.
        ...
        # Use the native SSP grid (5-year) consistently; do not coarsen to 10-year
        ...
        # Build absolute calendar years for decision times (base year = 2020)
        self.decision_years = 2020 + np.asarray(self.tree.decision_times, dtype=int)

        # Map decision years to indices on self.times using searchsorted to avoid
        # assumptions about DELTA_T and to handle any uniform spacing (5y, 10y, ...)
    ...
    def _make_extension(self, time, baseline_gtco2_2100, extension_year):
        ...
        # get time difference (= 5 in our case)
        ...
        # so just make that without the above headache
            ...
            try:
                if baseline == "cumemit":
                    ...
                    mitigated_baseline = self.CUMEMIT_2019 + 1e-3 * get_integral_var_ub(
