...
class BPWCost(Cost):
    ...
    def cost(self, period, mitigation, ave_mitigation):
        ...
        years_since_2020 = self.tree.decision_times[period]
        abs_year = 2020 + years_since_2020
    ...
    def price(self, years, mitigation, ave_mitigation):
        ...
        if np.any(over_mit):
            if np.ndim(mitigation) == 0:
                price = tech_term * (self.tau_0 + self.backstop_premium) * (np.exp(mitigation*self.power) - 1)
            else:
                price = tech_term * (self.tau_0 + self.backstop_premium) * (np.exp(mitigation[over_mit]*self.power) - 1)
