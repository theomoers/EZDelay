...
class OutputUnpacker:
    ...
    def __init__(self, filename, description, _type, tree, emit_baseline,
        ...
        # make full time series of emissions and temperature. 
        # We need to get the actual size returned by get_mitigated_baseline
        # Test with zero mitigation to get the correct array size
        # Use the first final state node (last period)
        test_node = self.tree.num_decision_nodes - self.tree.num_final_states
        test_m = np.zeros(self.tree.num_decision_nodes)
        test_emis, _ = self.emit_baseline.get_mitigated_baseline(test_m, node=test_node, baseline='gtco2')
        emis_time_size = len(test_emis)
        
        self.temp_full = np.zeros((self.tree.num_final_states, emis_time_size))
        self.emis_gtco2_full = np.zeros((self.tree.num_final_states, emis_time_size))
        ...
        if self._type == 'pickle':
            ...
            # col = 4 is ghg concentrations (in new format)
            # Try to load the data and detect the format
            # Calculate skip_footer based on number of periods (2 blank + 1 header + num_periods data lines)
            skip_footer = 2 + 1 + self.tree.num_periods
            try:
                # New format: Node;Mitigation;Prices;Average Mitigation;GHG Level
                self.output_data = np.genfromtxt(self.filename, delimiter=';',
                                                 usecols=(1,2,4), skip_header=1,
                                                 skip_footer=skip_footer)
            except (ValueError, IndexError):
                # Old format with col 5 for GHG
                self.output_data = np.genfromtxt(self.filename, delimiter=';',
    ...
    def _make_temp_emis_path(self):
        ...
        # loop through final states (e.g., 31 -> 63 for 7 periods, 255 -> 511 for 9 periods)
        final_state_start = self.tree.num_decision_nodes - self.tree.num_final_states
        for node in range(final_state_start, self.tree.num_decision_nodes):

            ind = node - final_state_start
    ...
    def _get_node_to_path(self, array):
        ...
        for node in range(final_state_start, self.tree.num_decision_nodes):
            ...
            path_array[node - final_state_start, :] = array[tmp_path]
        return path_array
